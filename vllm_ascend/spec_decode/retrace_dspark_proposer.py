...
"""Request-scoped ReTrace state on the native DSpARK execution path."""

import torch
from vllm.logger import init_logger
from vllm.model_executor.models.retrace_dspark import ReTraceRequestCache
...
def is_retrace_dspark_draft(speculative_config):
    draft = getattr(speculative_config, "draft_model_config", None)
    return draft is not None and "ReTraceDSparkDraftModel" in getattr(
        draft.hf_config, "architectures", []
    )


class AscendReTraceDSparkProposer(AscendDSparkProposer):
    def __init__(self, vllm_config, device, runner=None):
        ...
        if self.num_speculative_tokens != config.block_size:
            raise ValueError(
                "DSpARK anchor sampling requires block_size speculative tokens"
            )
        ...
        if vllm_config.speculative_config.enable_adaptive_verification:
            raise ValueError(
                "ReTrace-on-DSpARK currently uses fixed verification lengths"
            )
        if getattr(config, "dspark_draft_topk", None) is not None:
            raise ValueError(
                "ReTrace-on-DSpARK currently requires full-vocabulary sampling"
            )
        ...
        logger.info(
            "ReTrace-on-DSpARK: K=%d, Markov rank=%d, confidence head=%s, conditioning=%s",
            self.num_speculative_tokens,
            config.markov_rank,
            config.enable_confidence_head,
            self.retrace_enabled,
        )
    ...
    def retrace_capture(self, hidden_states, token_indices):
        ...
        positions = self.positions[indices].reshape(n, self.num_speculative_tokens) + 1
